		...
		/*namespace contour
		{
			struct DeduceCoordTypeFP16
			{
			  ...
			  void operator()(
			    ...
			    vtkm::cont::StorageTagUniformPoints>& coords,
				...
				if (this->GenerateNormals && generateHighQualityNormals)
  				{
					outputCells = this->Worklet.Run(ivalues, 
							vtkm::filter::ApplyPolicyCellSet(
								cells, policy, *this),
                                        coords.GetData(),field,vertices,normals);
